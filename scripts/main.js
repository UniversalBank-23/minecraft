...
system.breakBlockAt = function(x, y, z) {
    ...
    let block = this.getBlock(x, y, z); // Get the block at the coordinates
...
// Function to get the block at the given coordinates (using Minecraft scripting API)
system.getBlock = function(x, y, z) {
    // Fetching block information using the Minecraft scripting API is done via `getBlockAt()`
    let block = this.getBlockAt({ x: x, y: y, z: z });
    return block;
};
